...
- :ref:`Customer lead time <inventory/warehouses_storage/sales-customer-lt>`: The number of days
  ...
  This is the default time frame for customer order fulfillment, also known as *delivery lead time*.
...
The *customer lead time* is individually configured for each product. To set the customer lead time
for a product, navigate to :menuselection:`Sales app --> Products --> Products`. Select the desired
product, and switch to the :guilabel:`Inventory` tab. In the :guilabel:`Customer Lead Time` field,
enter the number of calendar days required to fulfill the delivery order from start to finish.
...
Configuring the days required to gather the components for a manufactured product creates |MOs| in
advance, and ensures there is enough time to either replenish components, or manufacture
semi-finished products.

.. note::
   If the longest component lead time is for a purchase component, the difference between *days to
   prepare manufacturing order* and *manufacturing lead time* is accounted for in the vendor lead
   time.
...
the necessary operations, based on the outgoing shipment delivery date, August 11:
